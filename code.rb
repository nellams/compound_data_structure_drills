...
people_with_kids = []
people.each do |a|
  if a[1].include?("children")
    people_with_kids << a[0]
  end
end
puts people_with_kids
# # * What are the names of the people who do not have children?
people_without_kids = []
people.each do |a|
  if !a[1].include?("children")
    people_without_kids << a[0]
  end
end
puts people_without_kids
...
# puts sets_of_people[0].include?("Ruby Hessel")
...
sets_of_people[0].each do |person, info|
  info.each do |k, v|
    if k == "company" && v.include?("m") || v.include?("M")
      companies_with_m << person
    end
  end
end
puts companies_with_m
